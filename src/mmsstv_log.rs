...
/// 1レコードのサイズ = SDMMLOG構造体そのもの(256byte)。
///
/// 訂正(2026-07-22): 012時点では「256byte本体+16byte索引領域」と推測していたが、
/// 実データ(4レコード, ヘッダー除いて1024byte)がRECORD_SIZE=256で
/// きれいに割り切れる(272では割り切れない)ことから、その仮説は誤りと判明。
/// 012当時に見えていた「レコード末尾の16byteにコールサインが重複している」
/// ように見えた現象は、実際には「次のレコードの本体(の先頭7バイト目、
/// つまりCallフィールド)」を見ていただけだった。
